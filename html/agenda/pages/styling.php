...
echo "<div class='butHead floatC noPrint'>
	<button title='{$prevTit}' type='submit' name='prev' value='y'>{$prevBut}</button><pre> </pre>";
if (empty($_SESSION[$calID]['theme'])) { //no preview mode
	echo "<button title='{$ax['sty_save_theme_title']}' type='submit' name='save' value='y'>{$ax['sty_save_theme']}</button><pre> </pre>\n";
	echo "<button title='{$ax['sty_backup_theme_title']}' type='submit' name='bkup' value='y'>{$ax['sty_backup_theme']}</button><pre> </pre>\n";
	$fNames = array(); //init
	$dirScan = scandir('./css');

...
<input type='radio' name='dFormat' id='dmy' value='d-m-y'".($dFormat == 'd-m-y' ? " checked" : '')."><label for='dmy'>{$ax['dd_mm_yyyy']}</label><pre>  </pre>
<input type='radio' name='dFormat' id='mdy' value='m-d-y'".($dFormat == 'm-d-y' ? " checked" : '')."><label for='mdy'>{$ax['mm_dd_yyyy']}</label><pre>  </pre>
...
<input type='radio' name='tFormat' id='hma' value='h:ma'".($tFormat == 'h:ma' ? " checked" : '')."><label for='hma'>{$ax['time_format_us']}</label><pre>  </pre>
...
function processUpload() {
	...
	foreach ($csvArray as $csvEvent) {
		...
		$csvFields = str_getcsv($csvEvent, $_POST['delimiter']); //get csv fields
...
<td><textarea cols='30' rows='1' name='descr[]'>{$_POST['descr'][$i]}</textarea></td>
	...
	echo "</table>
<br>
<button type='submit' name='addEvents' value='y'>{$ax['iex_add_events']}</button>

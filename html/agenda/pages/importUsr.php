...
function displayUsers($errors) {
	...
	if ($nofErrors) {
		echo "<p><span class='bar inputError'>&nbsp;</span> : {$ax['iex_invalid']}<pre>  </pre><span class='bar inputWarning'>&nbsp;</span> : {$ax['iex_in_use']}</p>";
	} else {
		echo "<p>&nbsp;</p>";
	}
...
<td><input type='text' size='45' name='uPwrd[]' value='{$_POST['uPwrd'][$i]}'></td>\n";
	...
	echo "</table>
<br>\n";
echo "<button type='submit' name='addUsers' value='y'>{$ax['iex_add_users']}</button>\n";

<?php
/* LuxCal display2 © 2009-2020 LuxSoft www.LuxSoft.eu*/$LDV='2020-03-30';
/*
...
$calName = "Upcoming in Black"; //calendar header text. Asterisk ("*"): calendar title; blank (""): no header
...
$evtHead = "#e"; //event head template (#e: event title, #c: event title - category color, #u: event title - owner group color, #o: event owner, #a: age, #/: new line)
...
$MTIME = "0px 0px 0px 0px"; //time margins - top right bottom left
...
$BBORD = "#111111"; //container background color
$CBORD = "#CF0209"; //container border color
$BDATE = "#111111"; //calendar date background color
...
$CEVNT = "#606070"; //event description color
$CTIME = "#AAAAAA"; //event time color
...
function makeEvtHead($head,$evt,$date) { //make event head
	...
	$keys = array('#e', '#c', '#u', '#o', '#a', '#/', '|'); //possible template keys
	$html = array($evt['tit'], "<span{$cStyle}>{$evt['tit']}</span>", "<span{$uStyle}>{$evt['tit']}</span>", $evt['una'], $age, "<br>", ""); //html code
	return str_replace($keys,$html,$head);
}

function showEvents($date) { //show events in calendar
	global $calID, $xx, $evtList, $evtHead, $evtBody, $usr, $futEvts, $evtWin, $rxULink, $rxIMGTags;
...
<meta name="application-name" content="LuxCal V<?php echo LCV.' Display2 V'.$LDV?>">
...
td.date {font:{$FDATE}; color:{$CDATE}; background:{$BDATE}; padding:{$MDATE};}
td.time {width:{$WTIME}px; font:{$FEVNT}; color:{$CTIME}; background:{$BEVNT}; padding:{$MTIME};}
td.event {margin:{$MEVNT}; text-align:justify; text-justify:inter-word;}
td.noEvt {text-align:center; padding:30px 0; font:{$FNOEV};}
div.eHead {font:{$FETIT}; color:{$CTITL}; margin:{$MTITL};}
div.eBody {color:{$CEVNT}; margin:{$MBODY};}
...
if (!$msg) { //no login form
	if ($login) { //display login/out button
...
<div class='floatC'><button type='submit' class='bold' name='log' value='i'>{$xx['log_in']}</button>\n";
	...
	if ($evtList) {
		foreach ($evtList as $cDate => &$events) { //loop thru dates
			...
			echo "<tr>\n<td class='date{$cursor}'{$onclick} colspan='2'>".makeDate($cDate)."</td>\n</tr>\n"; //show date
